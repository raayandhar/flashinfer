...
size_t dispatchGemmClusterShapeSm100(__nv_bfloat16 const* A, __nv_bfloat16 const* B, T* D, int m,
                                     ...
                                     char* workspacePtr, size_t const workspaceBytes,
                                     cudaStream_t stream, bool use_transpose) {
  using namespace cute;

  switch (gemmConfig.cluster_shape) {
    case ClusterShape::ClusterShape_1x1x1:
      return use_transpose
                 ? genericBf16GemmKernelLauncherSm100Swap<T, arch, CTA_M_, CTA_N_, CTA_K_,
                                                          Shape<_1, _1, _1>, _1SM>(
                       A, B, D, m, n, k, b, gemmConfig, workspacePtr, workspaceBytes, stream)
                 : genericBf16GemmKernelLauncherSm100<T, arch, CTA_M_, CTA_N_, CTA_K_,
                                                      Shape<_1, _1, _1>, _1SM>(
                       A, B, D, m, n, k, b, gemmConfig, workspacePtr, workspaceBytes, stream);
      break;
...
size_t dispatchToArch(__nv_bfloat16 const* A, __nv_bfloat16 const* B, void* D, int m, int n, int k,
                      ...
                      size_t const workspaceBytes, cudaStream_t stream, bool use_transpose) {
  using arch = cutlass::arch::Sm100;

  switch (gemmConfig.tile_config_sm100) {
    case CutlassTileConfigSM100::CtaShape64x64x128B:
      return use_transpose ? dispatchGemmClusterShapeSm100<T, arch, 64, 64, 128>(
                                 A, B, static_cast<T*>(D), m, n, k, b, gemmConfig, workspacePtr,
                                 workspaceBytes, stream, use_transpose)
                           : dispatchGemmClusterShapeSm100<T, arch, 64, 64, 128>(
                                 B, A, static_cast<T*>(D), n, m, k, b, gemmConfig, workspacePtr,
                                 workspaceBytes, stream, use_transpose);
      break;
    case CutlassTileConfigSM100::CtaShape64x128x128B:
      return use_transpose ? dispatchGemmClusterShapeSm100<T, arch, 64, 128, 128>(
                                 A, B, static_cast<T*>(D), m, n, k, b, gemmConfig, workspacePtr,
                                 workspaceBytes, stream, use_transpose)
                           : dispatchGemmClusterShapeSm100<T, arch, 64, 128, 128>(
                                 B, A, static_cast<T*>(D), n, m, k, b, gemmConfig, workspacePtr,
                                 workspaceBytes, stream, use_transpose);
      break;
    case CutlassTileConfigSM100::CtaShape64x256x128B:
      return use_transpose ? dispatchGemmClusterShapeSm100<T, arch, 64, 256, 128>(
                                 A, B, static_cast<T*>(D), m, n, k, b, gemmConfig, workspacePtr,
                                 workspaceBytes, stream, use_transpose)
                           : dispatchGemmClusterShapeSm100<T, arch, 64, 256, 128>(
                                 B, A, static_cast<T*>(D), n, m, k, b, gemmConfig, workspacePtr,
                                 workspaceBytes, stream, use_transpose);
      break;
    case CutlassTileConfigSM100::CtaShape128x64x128B:
      return use_transpose ? dispatchGemmClusterShapeSm100<T, arch, 128, 64, 128>(
                                 A, B, static_cast<T*>(D), m, n, k, b, gemmConfig, workspacePtr,
                                 workspaceBytes, stream, use_transpose)
                           : dispatchGemmClusterShapeSm100<T, arch, 128, 64, 128>(
                                 B, A, static_cast<T*>(D), n, m, k, b, gemmConfig, workspacePtr,
                                 workspaceBytes, stream, use_transpose);
      break;
    case CutlassTileConfigSM100::CtaShape128x128x128B:
      return use_transpose ? dispatchGemmClusterShapeSm100<T, arch, 128, 128, 128>(
                                 A, B, static_cast<T*>(D), m, n, k, b, gemmConfig, workspacePtr,
                                 workspaceBytes, stream, use_transpose)
                           : dispatchGemmClusterShapeSm100<T, arch, 128, 128, 128>(
                                 B, A, static_cast<T*>(D), n, m, k, b, gemmConfig, workspacePtr,
                                 workspaceBytes, stream, use_transpose);
      break;
...
size_t CutlassBf16GemmRunner<T>::getWorkspaceSizeImpl(int m, int n, int k) {
  ...
  for (auto const& gemmConfig : gemmConfigs) {
    try {
      size_t curr_workspace_size = dispatchToArch<T>(
          nullptr, nullptr, nullptr, m, n, k, 1, gemmConfig, nullptr, 0, nullptr, use_transpose);
